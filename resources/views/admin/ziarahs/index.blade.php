...
@section('title', 'Data Lokasi Ziarah')
@section('content')
    <div class="px-4 sm:px-6 lg:px-8">
        <div class="sm:flex sm:items-center">
            <div class="sm:flex-auto">
                <h1 class="text-xl font-semibold text-gray-900">Data Lokasi Ziarah</h1>
                <p class="mt-2 text-sm text-gray-700">Daftar semua lokasi ziarah yang terdata di sistem.</p>
            </div>
            <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
                <a href="{{ route('admin.ziarahs.create') }}"
                                ...
                                @forelse ($ziarahs as $ziarah)
                                    <tr>
                                        <td
                                            ...
                                            {{ $ziarah->nama_lokasi }}</td>
                                        <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                            {{ Str::limit($ziarah->deskripsi, 60) }}</td>
                                        {{-- Bagian Aksi tetap sama --}}
                                        <td
                                            class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                                            <div class="flex justify-end gap-x-4">
                                                <a href="{{ route('admin.ziarahs.show', $ziarah->id) }}"
                                                    class="text-gray-500 hover:text-indigo-600">Detail</a>
                                                <a href="{{ route('admin.ziarahs.edit', $ziarah->id) }}"
                                                    class="text-indigo-600 hover:text-indigo-900">Edit</a>
                                                <form action="{{ route('admin.ziarahs.destroy', $ziarah->id) }}"
                                                    method="POST"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="3" class="px-3 py-4 text-center text-sm text-gray-500">Belum ada
                                            data lokasi ziarah.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
                    <div class="mt-4">{{ $ziarahs->links() }}</div>
                </div>
            </div>
        </div>
    </div>
