    ...
    <div class="px-4 sm:px-6 lg:px-8">
        <div class="sm:flex sm:items-center">
            <div class="sm:flex-auto">
                <h1 class="text-xl font-semibold text-gray-900">Data Stakeholder</h1>
                <p class="mt-2 text-sm text-gray-700">Daftar semua stakeholder yang terdata di sistem.</p>
            </div>
            <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
                <a href="{{ route('admin.stakeholders.create') }}"
                                ...
                                @forelse ($stakeholders as $stakeholder)
                                    <tr>
                                        ...
                                        <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                            {{ $stakeholder->jenis }}</td>
                                        <td
                                            class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                                            <div class="flex justify-end gap-x-4">
                                                <a href="{{ route('admin.stakeholders.show', $stakeholder->id) }}"
                                                    class="text-gray-500 hover:text-indigo-600">Detail</a>
                                                <a href="{{ route('admin.stakeholders.edit', $stakeholder->id) }}"
                                                    class="text-indigo-600 hover:text-indigo-900">Edit</a>
                                                <form action="{{ route('admin.stakeholders.destroy', $stakeholder->id) }}"
                                                    method="POST"
