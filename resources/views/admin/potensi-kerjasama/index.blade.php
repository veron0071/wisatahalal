    ...
    <div class="px-4 sm:px-6 lg:px-8">
        <div class="sm:flex sm:items-center">
            <div class="sm:flex-auto">
                <h1 class="text-xl font-semibold text-gray-900">Sebaran Potensi Kerjasama</h1>
                <p class="mt-2 text-sm text-gray-700">Daftar semua potensi kerjasama yang terdata.</p>
            </div>
            <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none"><a href="{{ route('admin.potensi-kerjasama.create') }}"
        ...
        <div class="mt-8 flex flex-col">
            <div class="-my-2 -mx-4 overflow-x-auto sm:-mx-6 lg:-mx-8">
                <div class="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
                                ...
                                @forelse ($items as $item)
                                    <tr>
                                        <td
                                            ...
                                            {{ $item->kategori_mitra }}</td>
                                        <td class="px-3 py-4 text-sm text-gray-500">
                                            {{ Str::limit($item->mitra_potensial, 40) }}</td>
                                        <td class="px-3 py-4 text-sm text-gray-500">
                                            {{ Str::limit($item->isu_need_utama, 40) }}</td>
                                        <td
                                            class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                                            <div class="flex justify-end gap-x-4">
                                                <a href="{{ route('admin.potensi-kerjasama.show', $item->id) }}"
                                                    class="text-gray-500 hover:text-indigo-600">Detail</a>
                                                <a href="{{ route('admin.potensi-kerjasama.edit', $item->id) }}"
                                                    class="text-indigo-600 hover:text-indigo-900">Edit</a>
                                                <form action="{{ route('admin.potensi-kerjasama.destroy', $item->id) }}"
                                                    method="POST"
                    ...
                    <div class="mt-4">{{ $items->links('vendor.pagination.tailwind-admin') }}</div>
                </div>
            </div>
        </div>
    </div>
