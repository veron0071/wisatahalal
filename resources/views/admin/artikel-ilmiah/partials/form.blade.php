<div class="space-y-6">
    <div>
        <label for="nama" class="block text-sm font-medium text-gray-700">Nama Artikel</label>
        <input type="text" name="nama" id="nama"
            class="mt-1 block w-full bg-gray-50 border border-gray-300 rounded-md shadow-sm py-2 px-3"
            value="{{ old('nama', $artikel->nama ?? '') }}" required>
    </div>
    <div>
        <label for="file_artikel" class="block text-sm font-medium text-gray-700">Upload File (PDF/Word)</label>
        <div class="mt-1">
            <input
                ...
                id="file_artikel" name="file_artikel" type="file">
        </div>
        <p class="mt-2 text-xs text-gray-500">Kosongkan jika tidak ingin mengubah file.</p>
        @if (isset($artikel) && $artikel->file_artikel)
            <div class="mt-4">
...
<div class="mt-8 pt-5 border-t border-gray-200">
    <div class="flex justify-end gap-3">
        <a href="{{ route('admin.artikel-ilmiah.index') }}"
