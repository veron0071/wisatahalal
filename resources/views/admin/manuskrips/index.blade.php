...
@section('title', 'Arsip Manuskrip')
@section('content')
    <div class="px-4 sm:px-6 lg:px-8">
        <div class="sm:flex sm:items-center">
            <div class="sm:flex-auto">
                <h1 class="text-xl font-semibold text-gray-900">Arsip Manuskrip</h1>
                <p class="mt-2 text-sm text-gray-700">Daftar semua manuskrip yang diarsipkan dalam sistem.</p>
            </div>
            <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
                <a href="{{ route('admin.manuskrips.create') }}"
                    ...
                    <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
                        <table class="min-w-full divide-y divide-gray-300">
                            ...
                            <tbody class="divide-y divide-gray-200 bg-white">
                                @forelse ($manuskrips as $manuskrip)
                                    <tr>
                                        <td
                                            class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6">
                                            {{ $manuskrip->judul }}</td>
                                        <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                            {{ Str::limit($manuskrip->deskripsi, 60) }}</td>
                                        <td
                                            ...
                                            <div class="flex justify-end gap-x-4">
                                                ...
                                                <a href="{{ route('admin.manuskrips.show', $manuskrip->id) }}"
                                                    class="text-gray-500 hover:text-indigo-600">Detail</a>

                                                <a href="{{ route('admin.manuskrips.edit', $manuskrip->id) }}"
                                                    class="text-indigo-600 hover:text-indigo-900">Edit</a>
                                                <form action="{{ route('admin.manuskrips.destroy', $manuskrip->id) }}"
                                                    method="POST"
                                                    onsubmit="return confirm('Yakin ingin menghapus arsip ini?')">@csrf
                                ...
                                @empty
                                    <tr>
                                        <td colspan="3" class="px-3 py-4 text-center text-sm text-gray-500">Belum ada
                                            manuskrip yang diarsipkan.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
                    <div class="mt-4">{{ $manuskrips->links() }}</div>
                </div>
            </div>
        </div>
    </div>
