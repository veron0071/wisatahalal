    ...
    <div class="px-4 sm:px-6 lg:px-8">
        <div class="sm:flex sm:items-center">
            <div class="sm:flex-auto">
                <h1 class="text-xl font-semibold text-gray-900">Galeri Video</h1>
                <p class="mt-2 text-sm text-gray-700">Daftar semua video yang terdata di sistem.</p>
            </div>
                                    ...
                                    <tr>
                                        <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
                                            @php
                                                $thumbnail = '';
                                                if (
                                                    strpos($video->video_url, 'youtube.com') !== false ||
                                                    strpos($video->video_url, 'youtu.be') !== false
                                                ) {
                                        ...
                                        <td class="whitespace-nowrap px-3 py-4 text-sm font-medium text-gray-900">
                                            {{ $video->judul }}</td>
                                        <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ $video->kategori }}
                                        </td>
                                        <td
                                            ...
                                            <div class="flex justify-end gap-x-4">
                                                ...
                                                <form action="{{ route('admin.videos.destroy', $video->id) }}"
                                                    method="POST"
                                                    onsubmit="return confirm('Yakin ingin menghapus video ini?')">@csrf
